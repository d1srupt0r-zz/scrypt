...
namespace scrypt.CommandLine
{
    public class Options
    {
        public static List<Param> List = new List<Param> {
            ...
            ,new Param(3, new [] { "/x", "/hex" }, (x, k) => x.Hex(), "He[x] encode or decode text", Enums.ParamType.Command)
            ,new Param(4, new [] { "/f", "/flip" }, (x, k) => x.Flip(), "Execute character [f]lip on text", Enums.ParamType.Trigger)
            ,new Param(5, new [] { "/t", "/twist" }, (x, k) => x.Twist(), "Run [t]wist on text (alternating case)", Enums.ParamType.Trigger)
            ,new Param(6, new [] { "/c", "/cipher" }, (x, k) => x.Cipher(k), "Run [c]ipher on text using a [k]ey (default Z:W)", Enums.ParamType.Crypto)
